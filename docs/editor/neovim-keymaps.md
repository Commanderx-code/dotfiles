...
source: ~/dotfiles/configs/nvim/lua/config/keymaps.lua
runtime: ~/.config/nvim/lua/config/keymaps.lua
tags: neovim keymaps lazyvim
status: active
criticality: normal
...
Important custom mappings include `jj` to escape insert mode, insert-mode Ctrl-H/J/K/L movement, terminal Ctrl-Q, `<leader>tb` terminal split, `<leader>fw` live grep, `Alt-I` LazyVim terminal, `<leader>th` colorscheme picker, Cody mappings, and note/journal finders.
...
## Review note
The Cody mappings currently contain `remap = ture` exactly as written in the live source. If those mappings behave unexpectedly, review that spelling.
