---
title: Fish Alias Reference
category: Commands
managed_by: Home Manager
source: ~/dotfiles/configs/fish/conf.d/aliases.fish
runtime: ~/.config/fish/conf.d/aliases.fish
tags: fish aliases abbreviations
status: active
criticality: normal
last_verified: 2026-08-26
---
# Fish Alias Reference

## Important behavior
Plain `find` is aliased to `fd`, so GNU options such as `-maxdepth` require `command find` or `cfind`.

Interactive abbreviations include `up`, `ug`, `fu`, `fud`, `cl`, and `tg` for update workflows.
